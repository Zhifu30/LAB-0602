...
const EquipmentDetailModal: React.FC<EquipmentDetailModalProps> = ({
  ...
  const fetchEquipmentTypes = useCallback(async () => {
    try {
      ...
      setEquipmentTypes((data || []).map((row: any) => ({
        id: row.id,
    ...
    try {
      ...
      if (data) {
        setTypeResource({
          ...
          sharedSopFiles: data.shared_sop_files as { url: string; name: string; }[] | null
        });
      }
    } catch (error) {
      console.error('Error fetching type resource:', error);
    }
  }, [equipment.type]);
  ...
  useEffect(() => {
    fetchEquipmentTypes();
    fetchTypeResource();
  }, [fetchEquipmentTypes, fetchTypeResource, maintenanceSchedulesKey]);

  const handleSave = async () => {
    try {
      console.log('🔧 Modal Save — editedEquipment:', editedEquipment);
      await onUpdate(editedEquipment);
      // 同步更新 ViewForm 显示的数据源
      setEditedEquipment(editedEquipment);
      // 直接覆盖父组件传入的 equipment，让 ViewForm 显示最新数据
      Object.assign(equipment, editedEquipment);
      setIsEditing(false);
    } catch (err) {
  ...
  const handleDelete = () => {
    if (window.confirm('确定要删除这台仪器吗？此操作不可撤销。')) {
  ...
  const handleFaultReport = async (faultData: FaultReportData) => {
    try {
      ...
      await supabase.from('fault_reports').insert({
        ...
        reporter_name: profile?.username || 'Unknown',
        reason: faultData.reason,
        custom_reason: faultData.customReason,
        description: faultData.description,
        image_url: faultData.imageUrl
      });
      ...
      await supabase.functions.invoke('send-equipment-notification', {
        body: {
          ...
          status: 'out-of-order',
          reporterName: profile?.username || 'Unknown',
          reason: faultData.reason,
          description: faultData.description,
          imageUrl: faultData.imageUrl,
          adminEmail: 'zhifu.feng@brightfuture.com.hk'
        }
      });
  ...
  const handleScrapConfirm = async (password: string, reason: string) => {
    try {
      ...
      await supabase.from('scrap_records').insert({
        equipment_id: equipment.id,
        scrapped_by: profile?.user_id,
        scrapper_name: profile?.username || 'Unknown',
        reason: reason,
  ...
  const DetailContent = () => (
    <div className="flex flex-col overflow-hidden rounded-lg relative" style={{ minHeight: '480px', maxHeight: '85vh' }}>
      {backgroundImageUrl && (
        <div
      ...
      <div className="relative flex flex-col flex-1">
        ...
        <div className="shrink-0 relative">
          <div className="p-3 text-white">
            <div className="flex items-center justify-between mb-2">
              <div
                className={`flex items-center gap-2 px-3 py-1.5 rounded-lg ${!readOnly ? 'cursor-pointer hover:opacity-90' : ''} transition-opacity ${statusColors[equipment.status]}`}
                onClick={() => !readOnly && setShowStatusModal(true)}
                title={readOnly ? undefined : "点击更改状态"}
              >
                <span className="text-lg">{statusIcons[equipment.status]}</span>
                <h2 className={`font-semibold truncate ${embedded ? 'text-sm' : 'text-base'}`}>
                  {equipment.name}
                </h2>
              </div>
              <div className="flex items-center gap-1.5">
                {(equipment.sopFileUrl || (typeResource?.sharedSopFiles && typeResource.sharedSopFiles.length > 0)) && (
                  <Button
                    ...
                    onClick={() => {
                      if (equipment.sopFileUrl) {
                        window.open(equipment.sopFileUrl, '_blank');
                      } else if (typeResource?.sharedSopFiles?.[0]) {
                        window.open(typeResource.sharedSopFiles[0].url, '_blank');
                      }
                    }}
                ...
                {!readOnly && (
                  <Button
                    ...
                    className={`h-8 w-8 p-0 ${showQRCode ? 'bg-primary text-white' : 'bg-blue-500 hover:bg-blue-600 text-white'} border-0`}
                ...
                <Button
                  variant={backgroundImageUrl ? "secondary" : "ghost"}
                  size="sm"
                  onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-3">
          <div className="space-y-4 text-white">
            {showQRCode && (
              <div className="border border-white/20 rounded-lg p-2 bg-white/10 backdrop-blur-sm">
              ...
              <div className="flex flex-wrap gap-1 bg-white/10 backdrop-blur-sm rounded-lg p-2 border border-white/20">
                ...
                {typeResource.sharedSopFiles.slice(1).map((file, index) => (
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className="h-6 text-xs bg-emerald-500/80 border-emerald-400/50 text-white hover:bg-emerald-500"
            ...
            <MaintenanceScheduleManager
              ...
              equipmentResponsibleEmail={equipment.responsible_email}
              onScheduleChange={() => setMaintenanceSchedulesKey(prev => prev + 1)}
              readOnly={readOnly}
            />
          </div>
        </div>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[200] p-4">
      <div className="w-full max-w-xl min-h-[480px] max-h-[85vh]">
        <DetailContent />
      </div>
    </div>
  );
    ...
    <div className="space-y-4 bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20">
      <div className="flex justify-between items-center">
        ...
        {!readOnly && isAdmin() && (
          <div className="flex gap-2">
            <Button onClick={onEdit} size="sm" className="h-7 bg-blue-500 hover:bg-blue-600 text-white">
              <Edit className="h-3 w-3 mr-1" />
              编辑
            </Button>
            <Button onClick={onDelete} size="sm" variant="destructive" className="h-7">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white/5 rounded-lg p-2">
          <Label className="text-xs text-white/80">仪器编号</Label>
          <p className="text-white text-base font-bold">{equipment.id}</p>
        </div>
        <div className="bg-white/5 rounded-lg p-2">
          <Label className="text-xs text-white/80">仪器名称</Label>
          <p className="text-white text-base font-bold">{equipment.name}</p>
        </div>
        <div className="bg-white/5 rounded-lg p-2">
          <Label className="text-xs text-white/80">设备类型</Label>
          <p className="text-white text-base font-bold">{equipment.type || '未分类'}</p>
        </div>
        <div className="bg-white/5 rounded-lg p-2">
          <Label className="text-xs text-white/80">型号</Label>
          <p className="text-white text-base font-bold">{equipment.model}</p>
        </div>
        <div className="bg-white/5 rounded-lg p-2">
          <Label className="text-xs text-white/80">厂商</Label>
          <p className="text-white text-base font-bold">{equipment.manufacturer}</p>
        </div>
        <div className="bg-white/5 rounded-lg p-2">
          <Label className="text-xs text-white/80">位置</Label>
          <p className="text-white text-base font-bold">{equipment.location}</p>
        </div>
        <div className="bg-white/5 rounded-lg p-2">
          <Label className="text-xs text-white/80">负责人</Label>
          <p className="text-white text-base font-bold">{equipment.responsible}</p>
        </div>
        <div className="bg-white/5 rounded-lg p-2">
          <Label className="text-xs text-white/80">下次校正日期</Label>
          <p className={`text-base font-bold flex items-center gap-1 ${getDateUrgencyColor(equipment.nextCalibrationDate)}`}>
            <Calendar className="h-3.5 w-3.5" />
            {equipment.nextCalibrationDate || '未设置'}
          </p>
        </div>
      </div>
    </div>
...
const EditForm: React.FC<{
  ...
  const handleResponsibleChange = (username: string) => {
    ...
    if (selectedUser) {
      onChange('responsible', selectedUser.username);
      if (selectedUser.email) {
        onChange('responsible_email', selectedUser.email);
      }
    }
  };
  ...
  return (
    <div className="space-y-6 bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <Button onClick={onSave} size="sm" className="bg-green-600 hover:bg-green-700 text-white">
            保存
          </Button>
          <Button onClick={onCancel} size="sm" variant="outline" className="bg-white/20 hover:bg-white/30 text-white border-white/30">
            取消
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white/5 rounded-lg p-2">
          <Label htmlFor="edit-name" className="text-xs text-white/80">仪器名称</Label>
          <Input
            id="edit-name"
        ...
        <div className="bg-white/5 rounded-lg p-2">
          <Label htmlFor="edit-type" className="text-xs text-white/80">设备类型</Label>
          <Select
          ...
          >
            ...
            <SelectContent className="z-[300] bg-popover">
              {equipmentTypes.length > 0 ? (
                ...
                <SelectItem value="__empty__" disabled>请先在设备类型管理中添加类型</SelectItem>
              )}
            </SelectContent>
          </Select>
        </div>
        <div className="bg-white/5 rounded-lg p-2">
          <Label htmlFor="edit-model" className="text-xs text-white/80">型号</Label>
          <Input
            id="edit-model"
        ...
        <div className="bg-white/5 rounded-lg p-2">
          <Label htmlFor="edit-manufacturer" className="text-xs text-white/80">厂商</Label>
          <Input
        ...
        <div className="bg-white/5 rounded-lg p-2">
          <Label htmlFor="edit-status" className="text-xs text-white/80">状态</Label>
          <Select
            value={equipment.status}
            onValueChange={(value: EquipmentStatus) => onChange('status', value)}
          >
            <SelectTrigger className="bg-white/10 border-white/20 text-white">
              <SelectValue />
            </SelectTrigger>
        ...
        <div className="bg-white/5 rounded-lg p-2">
          <Label htmlFor="edit-location" className="text-xs text-white/80">位置</Label>
          <Input
            id="edit-location"
        ...
        <div className="bg-white/5 rounded-lg p-2">
          <Label htmlFor="edit-responsible" className="text-xs text-white/80">负责人</Label>
          <Select
        ...
        <div className="bg-white/5 rounded-lg p-2">
          <Label htmlFor="edit-nextCalibrationDate" className="text-xs text-white/80">下次校正日期</Label>
          <Input
            id="edit-nextCalibrationDate"
            type="date"
        ...
        <div className="md:col-span-2 bg-white/5 rounded-lg p-2">
          <Label className="text-xs text-white/80">维护日期管理</Label>
          <p className="mt-1 text-xs text-white/60 bg-white/5 p-2 rounded">
            维护日期通过下方的"维护计划管理"进行设置和管理，支持自动周期性更新。
          </p>
        </div>
      </div>

      <div className="bg-white/5 rounded-lg p-2">
        <Label htmlFor="edit-description" className="text-xs text-white/80">描述</Label>
        <Textarea
          id="edit-description"
      ...
      <ImageUploader
        ...
        onImageChange={(imageUrl) => onChange('imageUrl', imageUrl)}
